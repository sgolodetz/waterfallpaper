...
\begin{enumerate}
\item Recursively process any children (the fact that the children are processed first is why this is called an `up' pass). (Recurse on every node in $C_N(n)$, the node children of $n$.)
\item If this node has a unique edge of steepest descent (i.e.~a unique lowest-valued edge leading out of it, which can be the parent edge), add an arrow on the node pointing along the edge. (Let $\mbox{steepest\_edges}(n) = \arg\min_{e \in \mbox{adjacent\_edges}(n)} w(e)$. If $|\mbox{steepest\_edges}(n)| = 1$, $\mbox{arrows}(n) \leftarrow \{\mbox{far\_end}(e) | e \in \mbox{steepest\_edges}(n)\}$.)
