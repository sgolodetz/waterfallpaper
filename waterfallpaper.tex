...
Let $T = (N,E)$ be the rooted MST on which the method is to operate, with node set $N = (n_i)$ and edge set $E = \{(n_p,n_c)\}$ such that $n_p$ is the parent of $n_c$. We define $C(e)$ to be the children of edge $e$, i.e.
%
\[
C(e) = \{(n_c,\_) \; | \; e = (n_p,n_c)\}
\]
...
Each pass of the method is then invoked on the root edge and proceeds recursively as follows (in which we denote the current edge as $e$):
%
\begin{enumerate}
...
\item We recurse on the children of $e$ (i.e.~the edges in $C(e)$).

\item A child edge with minimum value is chosen (if one or more of the children with minimum value is a guard edge, one of those is chosen in preference to a non-guard). This is then referred to as the `lowest child'. The value on the current edge is compared to the value on the lowest child. If its value is less than that of the lowest child, it is marked as a non-guard edge; otherwise it is marked as a guard and the lowest child is marked as a non-guard. (Let $\mbox{SC}(e)$ be the result of sorting $C(e)$ first by value, lowest first, and then by flag (G before N), and let $\pi_1,...,\pi_k$ be the corresponding permutation of $1,...,k$, that is

\[
\mbox{SC}(e) = (c_{\pi_1},...,c_{\pi_k})
\]

with

\[
\forall i \cdot w(c_{\pi_i}) \le w(c_{\pi_{i+1}})
\]

and
\[
\forall i \cdot flag(c_{\pi_i}) = N \Rightarrow flag(c_{\pi_{i+1}}) = N
\]

Let $c_\ell = c_{\pi_1}$ be the (chosen) lowest child. If $w(e) \le w(c_\ell)$ then $\mbox{flag}(e) \leftarrow N$ else $\mbox{flag}(e) \leftarrow G$ and $\mbox{flag}(c_\ell) \leftarrow N$.)

\item All non-guard children are then contracted. (Contract each $c$ in $\mbox{SC}(e)$ such that $\mbox{flag}(c) = N$.)
