...
\caption{Finding and contracting a graph's local minima for Marcotegui's method: (a) the graph from \cite{marcotegui05} and its local minima (drawn in red); (b) contracting/colouring the local minima (the edges to be contracted are drawn in blue).}
...
\caption[The SW method in action]{The SW method in action (considering all the edges in each level at a time for space reasons): black edges are non-guards, red edges are guards, blue edges are those which have been contracted and green edges are ones under active consideration. Note that in this example, the SW method's treatment of non-minimal plateaux has resulted in a $5$ edge being retained when it would have been contracted by Marcotegui's method.}
