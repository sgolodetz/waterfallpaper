...
\caption{The first method starts by picking a node at which to root the MST}% and adding a dummy root edge above the chosen root}
...
\caption[The first method in action]{The first method in action (considering all the edges in each level at a time for space reasons): black edges are non-guards, red edges are guards, blue edges are those which have been elided and green edges are ones under active consideration.}
...
\caption[Case analysis for the edge elision step of the second method]{Case analysis for the edge elision step of the second method (a blue edge indicates that the edge would be elided; a red edge indicates that it wouldn't). The labels are AI = ambiguous in, AO = ambiguous out, NF = no flow, UI = unambiguous in and UO = unambiguous out.}
...
\caption{Case analysis for the first pass of the second method}
...
\caption[Case analysis for the resolution step of the second method]{Case analysis for the resolution step of the second method (the circled node is the one currently under consideration in each case)}
...
\caption[The second method running on a real example]{The second method running on a real example: the arrows on the nodes indicate the flow direction, blue edges are those that will be elided and red edges are those that won't be.}
...
In this paper, we have introduced two new, tree-based methods for the classical waterfall transform from mathematical morphology and compared them with both the existing state-of-the-art and each other. The first method is single-pass and is significantly easier to implement than the existing state-of-the art: indeed, it can be implemented using a single, short, recursive function. The second method is slightly harder to implement (although still easier than previous methods), but deals consistently with the problem of non-minimal plateaux in the MST, something neither of the two other methods attempts. TODO
