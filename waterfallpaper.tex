...
In order to evaluate how well the methods scale with increasing input size, we timed each of them when used to segment increasingly large chunks from a 3D abdominal CT scan\footnotemark{}. The overall scan was of size $512 \times 512 \times 132$, and we tested the methods on chunks of sizes $512 \times 512 \times n$, for various different values of $n$. There was an upper bound of $40$ on $n$ in our experiments due to memory constraints. As before, the image in each case was first smoothed and subjected to the Meijster/Roerdink watershed. The results are shown in Table~\ref{?}.
