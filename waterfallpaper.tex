...
\begin{enumerate}
\item \emph{Check upwards route.} If $n$ was marked as having a potential upwards path of steepest descent (i.e.~$checkParent(n)$ is true), first check to make sure that there is no arrow on the parent node pointing down the edge to this node (i.e.~$reverseArrow(p_E(n),n)$ is false). If there is no such arrow, then there is a possible route upwards with distance value $d' = 1 + d(p_N(n))$. This is compared to the cost of the best existing routes downwards via child edges (stored as $d(n)$). If the upwards route is strictly better (i.e.~$d' < d(n)$), then the upwards route replaces the downwards ones (i.e.~$A(n) \leftarrow \{p_N(n)\}$) and the distance value on the node is updated to reflect the better route (i.e.~$d(n) \leftarrow d'$). If the upwards route is equally good (i.e.~$d' = d(n)$), the parent edge is marked (i.e.~$A(n) \leftarrow A(n) \cup \{p_N(n)\}$) but no other changes are made. If the upwards route is worse (i.e.~$d' > d(n)$), it is simply ignored.
